...
export  function MyComponent() {
	...
	useEffect(()=>{
		...
		response?.then((res)=>{
			...
			console.log(res)
			if(res?.content){
				setUserTableId(res.content?.fields.users.fields.id.id)
			}
		}).catch((error)=>{
	...
	useEffect(()=>{
		if(userTableId && !userObject){
			const response = fetchDynamicFieldObject(
				...
				"0x1::string::String",
				"d790d41adfffd48df8e38607991a297970743decff87517e647008a652587d4c" // 用户地址
			);
			response?.then((res)=>{
				setLoading(true);
				console.log(222,res)
				if(res?.content){
					...
					setUserObject(res?.content.fields.value.fields) // 不知道为啥没有值
					setReportTableId(res?.content.fields.value.fields.reports.fields.id.id)
				}
			}).catch((error)=>{
				setError(error.message);
			}).finally (()=>{
